...
FixStyle(client/gcmc,FixClientGCMC)

#else

#ifndef LMP_FIX_CLIENT_GCMC_H
#define LMP_FIX_CLIENT_GCMC_H

#include "fix.h"

namespace LAMMPS_NS {

class FixClientGCMC : public Fix {
 public:
  FixClientGCMC(class LAMMPS *, int, char **);
  ~FixClientGCMC();
  int setmask();
  void init();
  void pre_exchange();
  ...
  void update_gas_atoms_list_accepted();
  void send_recv_update_request();
 ...
 private:
  ...
  void pack_coords();
  void pack_box();
  double receive_fev(int);
  double get_prev_energy();
  void storing_energy(double);
