...
/* ----------------------------------------------------------------------
   ...
   - this fix requires periodic boundary conditions for use in conjunction with VASP
   - this fix has primarily been written for performing GCMC of atoms, however, GCMC of molecules can possibly be done with minor edits to the code
   - code for this fix has largely been taken from fix_gcmc.cpp and fix_client_md.cpp
------------------------------------------------------------------------- */

// headers from fix_gcmc.cpp
#include "fix_client_gcmc.h"
#include <mpi.h>
#include <cmath>
#include <cstring>
#include "atom.h"
...
#include "error.h"
#include "neighbor.h"
// headers for I/O such as printing, writing to files, etc. in C++
#include <cstdio>   
#include <iostream>
#include <fstream>
using namespace std;
...
enum{SETUP=1,STEP,UPDATE}; 
enum{DIM=1,PERIODICITY,ORIGIN,BOX,NATOMS,NTYPES,TYPES,COORDS,UNITS,CHARGE}; 
enum{ENERGY=1,ERROR}; 

/* ---------------------------------------------------------------------- */

FixClientGCMC::FixClientGCMC(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),
...
{ 
  // largely from fix_client_md.cpp
  if (lmp->clientserver != 1)
    error->all(FLERR,"Fix client/gcmc requires LAMMPS be running as a client");
  if (!atom->map_style) error->all(FLERR,"Fix client/gcmc requires atom map");

  if (sizeof(tagint) != 4)
    error->all(FLERR,"Fix client/gcmc requires 4-byte atom IDs");

  if (strcmp(update->unit_style,"real") == 0) units = REAL;
  else if (strcmp(update->unit_style,"metal") == 0) units = METAL;
  else units = OTHER;

  scalar_flag = 1;                    // 1 if compute_scalar() function exists, 0 if not
  global_freq = 1;                    // frequency s/v data is available at
  extscalar = 1;                      // 0/1 if global scalar is intensive/extensive
  virial_flag = 0;                    // 1 if Fix contributes to virial, 0 if not
  thermo_virial = 1;                  // 1 if fix_modify enabled ThVir, 0 if not 
  ...
  econvert = 1.0;
  if (units == REAL) {
    econvert = 23.06035;    // eV -> Kcal/mole
  }

  // largely from fix_gcmc.cpp
  if (narg < 12) error->all(FLERR,"Illegal fix client/gcmc command");

  if (atom->molecular == 2)
    error->all(FLERR,"Fix client/gcmc does not (yet) work with atom_style template");

  dynamic_group_allow = 1;            // 1 if can be used with dynamic group, else 0

  vector_flag = 1;                    // 0/1 if compute_vector() function exists
  size_vector = 8;                    // length of global vector
  global_freq = 1;                    // frequency s/v data is available at
  extvector = 0;                      // 0/1/-1 if global vector is all int/ext/extlist
  restart_global = 1;                 // 1 if Fix saves global state, 0 if not
  time_depend = 1;                    // 1 if requires continuous timestepping

  // required args

  nevery = force->inumeric(FLERR,arg[3]);
  nexchanges = force->inumeric(FLERR,arg[4]); // % exchange moves
  nmcmoves = force->inumeric(FLERR,arg[5]); // % translation moves
  ...
  effective_volume = force->numeric(FLERR,arg[11]); // includes config bias if need be; if 0, region_volume calculated 

  if (nevery <= 0) error->all(FLERR,"Illegal fix client/gcmc command");
  if (nexchanges < 0) error->all(FLERR,"Illegal fix client/gcmc command");
  if (nexchanges > 100) error->all(FLERR,"Illegal fix client/gcmc command");
  if (nmcmoves < 0) error->all(FLERR,"Illegal fix client/gcmc command");
  if (nmcmoves > 100) error->all(FLERR,"Illegal fix client/gcmc command");
  if (seed <= 0) error->all(FLERR,"Illegal fix client/gcmc command");
  if (reservoir_temperature < 0.0)
    error->all(FLERR,"Illegal fix client/gcmc command");
  if (displace < 0.0) error->all(FLERR,"Illegal fix client/gcmc command");
  if (effective_volume < 0.0) error->all(FLERR,"Illegal fix client/gcmc command");
...
/*  // code below works only for the case in which fix client/gcmc region extends across periodic domain in x-y direction  
  // getting region volume as input 
  ...
  if (regionflag) {
    ...
    region_xlo = domain->regions[iregion]->xlo; // only if xlo == boxlo[0]
    region_xhi = domain->regions[iregion]->xhi; // only if xhi == boxhi[0]
    region_ylo = domain->regions[iregion]->ylo; // only if ylo == boxlo[1]
    region_yhi = domain->regions[iregion]->yhi; // only if yhi == boxhi[1]
    region_zlo = domain->regions[iregion]->extent_zlo; 
    region_zhi = domain->regions[iregion]->extent_zhi; 
    cout << region_xlo;cout<<"\n";cout << region_xhi;cout<<"\n";
    cout << region_ylo;cout<<"\n";cout << region_yhi;cout<<"\n";
    cout << region_zlo;cout<<"\n";cout << region_zhi;cout<<"\n";
    cout << domain->boxlo[0];cout<<"\n";cout << domain->boxhi[0];cout<<"\n";
    cout << domain->boxlo[1];cout<<"\n";cout << domain->boxhi[1];cout<<"\n";
    cout << domain->boxlo[2];cout<<"\n";cout << domain->boxhi[2];cout<<"\n";
    // calculating volume of prism using formula (area of rhombus * height)
    // region_volume = (region_xhi - region_xlo)*(region_xhi - region_xlo)*0.866*(region_zhi - region_zlo);
    region_volume = effective_volume; 
    printf("Estimated region volume is %f\n", region_volume);
  }*/
  ...
  if (regionflag) {
    ...
    // Commenting this out, as definition of extent_xlo, etc. is such that if fix client/gcmc region 
    // extends across domain boundary in x-y direction, region_xhi will be > domain->boxhi[0], 
    // resulting in an error call. But this does not make a difference when considering periodic boundaries.
    // However, region_volume calculation is still correct, as match(x,y,z) uses inside(x,y,z)
    // which uses xlo, xhi, etc. of region (not extent_xlo, extent_xhi, etc.)
...
/*    if (region_xlo < domain->boxlo[0] || region_xhi > domain->boxhi[0] ||
  ...
  force_reneighbor = 1; // 1 if Fix forces reneighboring, 0 if not
  next_reneighbor = update->ntimestep + 1; // next timestep to force a reneighboring
...
------------------------------------------------------------------------- */
// from fix_gcmc.cpp
void FixClientGCMC::options(int narg, char **arg)
{
  if (narg < 0) error->all(FLERR,"Illegal fix client/gcmc command");
  ...
  if (strcmp(arg[iarg],"mol") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix client/gcmc command");
      imol = atom->find_molecule(arg[iarg+1]);
      if (imol == -1)
        error->all(FLERR,"Molecule template ID for fix client/gcmc does not exist");
      if (atom->molecules[imol]->nset > 1 && comm->me == 0)
        error->warning(FLERR,"Molecule template for "
                       "fix client/gcmc has multiple molecules");
      exchmode = EXCHMOL;
      onemols = atom->molecules;
      nmol = onemols[imol]->nset;
      iarg += 2;
  } else if (strcmp(arg[iarg],"mcmoves") == 0) {
      if (iarg+4 > narg) error->all(FLERR,"Illegal fix client/gcmc command");
      ...
      if (iregion == -1)
        error->all(FLERR,"Region ID for fix client/gcmc does not exist");
      ...
      if (iarg+3 > narg) error->all(FLERR,"Illegal fix client/gcmc command");
...
int FixClientGCMC::setmask()
{
  // from fix_client_gcmc.cpp
  int mask = 0;
  mask |= PRE_EXCHANGE;
  mask |= THERMO_ENERGY; // from fix_client_md.cpp 
  return mask;
}

/* ---------------------------------------------------------------------- */

void FixClientGCMC::init()
{
  ...
  if (!full_flag) {
    if ((force->kspace) ||
      ...
      full_flag = true;
      if (comm->me == 0)
        error->warning(FLERR,"Fix client/gcmc using full_energy option");
    }
  }
  ...
  if (exchmode == EXCHATOM) {
    if (ngcmc_type <= 0 || ngcmc_type > atom->ntypes)
      error->all(FLERR,"Invalid atom type in fix client/gcmc command");
  }
  ...
  if ((exchmode == EXCHATOM) && atom->molecule_flag) {
    ...
    if (flagall && comm->me == 0)
      error->all(FLERR,
       "Fix client/gcmc cannot exchange individual atoms belonging to a molecule");
  }
  ...
  if (gas_mass <= 0.0)
    error->all(FLERR,"Illegal fix client/gcmc gas mass <= 0");
  ...
  // can compute these values here (instead of in attempt_atomic_insertion_full() etc., 
  // as we allow the insertion/deletion/translation of only one atom type)
  ...
  if ((comm->me == 0) && (groupbit & 1))
    error->warning(FLERR, "Fix client/gcmc is being applied "
  ...
  for (int igroup = 0; igroup < ngroups; igroup++) {
    ...
    if (jgroup == -1)
      error->all(FLERR,"Could not find specified fix client/gcmc group ID");
  ...
  if (full_flag && (exchmode == EXCHMOL || movemode == MOVEMOL) && comm->nprocs > 1)
    error->all(FLERR,"fix client/gcmc does currently not support full_energy option with molecules on more than 1 MPI process.");

  // from fix_client_md.cpp

  if (3*atom->natoms > INT_MAX)
    error->all(FLERR,"Fix client/gcmc max atoms is 1/3 of 2^31");
...
void FixClientGCMC::pre_exchange()
{
  ...
  //update_gas_atoms_list(); //already updating accordingly within setup and exchange functions

  if (full_flag) {
    energy_stored = get_prev_energy();
    printf("energy stored is %f\n", energy_stored);
...
/*    for (int i = 0; i < ncycles; i++) {
      ...
      if (ixm <= nmcmoves) {
        ...
        if (xmcmove < patomtrans) {printf("Performing translation attempt since ixm <= nmcmoves and xmcmove < patomtrans at %d, %f\n",ixm,xmcmove);attempt_atomic_translation_full();}
        ...
        if (exchmode == EXCHATOM) {
          if (xgcmc < 0.5) {printf("Performing delete attempt since xgcmc < 0.5 at %f\n", xgcmc);attempt_atomic_deletion_full();}
          else {printf("Performing insert attempt since xgcmc > 0.5 at %f\n", xgcmc);attempt_atomic_insertion_full();}
        } else {
    ...
    // nmcmoves % translation, nexchanges % exchange

    double prob_nmcmoves = static_cast<double>(nmcmoves)/100.0;
    double ixm = random_equal->uniform();
    if (ixm < prob_nmcmoves){printf("Performing translation attempt since ixm < %f at %f\n",prob_nmcmoves, ixm);attempt_atomic_translation_full();}
    else {
    printf("Performing exchange attempt since ixm >= %f at %f\n",prob_nmcmoves, ixm);
    double xgcmc = random_equal->uniform();    
    if (xgcmc < 0.5) {printf("Performing delete attempt since xgcmc < 0.5 at %f\n", xgcmc);attempt_atomic_deletion_full();}
    else {printf("Performing insert attempt since xgcmc >= 0.5 at %f\n", xgcmc);attempt_atomic_insertion_full();}	
    }
    ...
    if (triclinic) domain->lamda2x(atom->nlocal+atom->nghost);
  	...
  	error->all(FLERR,"fix client/gcmc can currently only compute the entire system energy");
...
// fix client/gcmc calcualtes only full energy, 
// therefore commenting out functions that do not compute full energy

/*void FixClientGCMC::attempt_atomic_translation()
{
...
/*void FixClientGCMC::attempt_atomic_deletion()
{
...
/*void FixClientGCMC::attempt_atomic_insertion()
{
  double lamda[3];
...
/*void FixClientGCMC::attempt_molecule_translation()
{
  ntranslation_attempts += 1.0;
...
/*void FixClientGCMC::attempt_molecule_rotation()
{
...
/*void FixClientGCMC::attempt_molecule_deletion()
{
  ndeletion_attempts += 1.0;
...
/*void FixClientGCMC::attempt_molecule_insertion()
{
  double lamda[3];
  ninsertion_attempts += 1.0;
...
void FixClientGCMC::attempt_atomic_translation_full()
{
  ntranslation_attempts += 1.0;
  ...
  // storing translated atom's type to calculate mu (just for printing purposes)
  int tmptype;
  tmptype = atom->type[i];

  // computing number of atoms being translated from, of the particular type (just for printing purposes)
  ...
  if (i >= 0) {
    ...
    //domain->remap(coord);
    //printf("Remapped coords are %f %f %f \n",coord[0], coord[1], coord[2]);    
    if (regionflag) {
      while (domain->regions[iregion]->match(coord[0],coord[1],coord[2]) == 0) {
        rsq = 1.1;
        while (rsq > 1.0) {
  ...
  double ran_num_trans = random_equal->uniform();

  if (energy_after < MAXENERGYTEST &&
      ran_num_trans <
      exp(beta*(energy_before - energy_after))) {

  	printf("Accepting move as random number %f is less than RHS %f\n",ran_num_trans, exp(beta*(energy_before - energy_after)));

    energy_stored = energy_after;
    ntranslation_successes += 1.0;
    printf("energy stored is %f\n", energy_stored);
    send_recv_update_request(); // update atom positions and gas list from CONTCAR
  } else {
  	printf("Rejecting move as random number %f is more than RHS %f\n",ran_num_trans, exp(beta*(energy_before - energy_after)));   
    ...
    printf("energy stored is %f\n", energy_stored);
    update_gas_atoms_list(); // update gas list to ensure only previous configuration atom list is accounted for     
  }

  printf("Storing energy\n");
  storing_energy(energy_stored);
}

/* ----------------------------------------------------------------------
------------------------------------------------------------------------- */

void FixClientGCMC::attempt_atomic_deletion_full()
{
  ...
  // storing deleted atom's coords, mask and type in tmp vars
  double xtmp[3];
  xtmp[0] = xtmp[1] = xtmp[2] = 0.0;
  int tmpmask;
  int tmptype;

  // storing all other atoms' coords, mask and type in temp vars
  double temp_x[atom->nlocal-1][3]; 
  int temp_type[atom->nlocal-1];
  int temp_mask[atom->nlocal-1];

  if (atom->natoms == 1) {
    error->warning(FLERR,"natoms = 1, will not perform deletion");
    energy_stored = energy_before;    
  } else {
    if (i >= 0) {
      // storing coords, mask, type in case trial is not accepted and need put atom back in its coords
      // deleting atom i while ensuring ordering for VASP remains intact
      // storing all other atoms' coords, mask and type in temp vars

      xtmp[0] = x[i][0]; 
      xtmp[1] = x[i][1];
      xtmp[2] = x[i][2];
      tmpmask = atom->mask[i]; //atom->mask array contains the bits that identify to which group an atom belongs to
      tmptype = atom->type[i];

      // copy atom m+1 info to atom m and deleting atom m
      // this copies atom tag, type, mask, image, coords, velocity, radius, rmass, angmom

      for (int m = i; m < atom->nlocal; m++){
        atom->avec->copy(m+1,m,1);
      }      
      atom->nlocal--;  
      atom->natoms--;

      tagint *tag = atom->tag;
      for (int j = 0; j < atom->nlocal; j++) tag[j] = 0;
      atom->tag_extend(); //reordering
      for (int k = 0; k < atom->nlocal; k++){
        temp_x[k][0] = x[k][0];
        temp_x[k][1] = x[k][1];
        temp_x[k][2] = x[k][2];
        temp_type[k] = atom->type[k];
        temp_mask[k] = atom->mask[k];
      }
    ...
    // compute beta, lambda, sigma, and the zz factor with correct gas_mass and chemical potential
/*    beta = 1.0/(force->boltz*reservoir_temperature);
    if (strcmp(update->unit_style,"lj") == 0)
      zz = exp(beta*mu);
    else {
      double lambda = sqrt(force->hplanck*force->hplanck/
                         (2.0*MY_PI*gas_mass*force->mvv2e*
                        force->boltz*reservoir_temperature));
      zz = exp(beta*mu)/(pow(lambda,3.0));
    }
    sigma = sqrt(force->boltz*reservoir_temperature*tfac_insert/gas_mass/force->mvv2e);
    if (pressure_flag) zz = pressure*fugacity_coeff*beta/force->nktv2p;*/

    double ran_num_del = random_equal->uniform();
    
    if (ran_num_del <
      count*exp(beta*(energy_before - energy_after))/(zz*volume)) {

      printf("Accepting move as random number %f is less than RHS %f\n",ran_num_del, count*exp(beta*(energy_before - energy_after))/(zz*volume));   

      ndeletion_successes += 1.0;
      energy_stored = energy_after;
      printf("energy stored is %f\n", energy_stored);
      send_recv_update_request(); 
    } else {
      if (i>=0) {
      	printf("Rejecting move as random number %f is more than RHS %f\n",ran_num_del, count*exp(beta*(energy_before - energy_after))/(zz*volume));
        // create atom at correct coord
        // correct ordering for VASP
        atom->avec->create_atom(tmptype,xtmp); //create atom of type tmptype at coords xtmp; this creates atom at end of list
        if (atom->tag_enable){
          atom->tag_extend();
          if (atom->map_style) atom->map_init();
        }
        atom->mask[atom->nlocal-1] = groupbitall;
        atom->mask[atom->nlocal-1] |= tmpmask; //atom->mask array contains the bits that identify to which group an atom belongs to
        atom->natoms++;
        x[i][0] = x[atom->nlocal-1][0]; //restoring coords at i
        x[i][1] = x[atom->nlocal-1][1];
        x[i][2] = x[atom->nlocal-1][2];  
        atom->type[i] = tmptype;
        atom->mask[i] = atom->mask[atom->nlocal-1];
        for (int l = i+1; l < atom->nlocal; l++) { 
          x[l][0] = temp_x[l-1][0];
          x[l][1] = temp_x[l-1][1];
          x[l][2] = temp_x[l-1][2];
          atom->type[l] = temp_type[l-1];
          atom->mask[l] = temp_mask[l-1];
        } //note: atoms have numerous attributes (tag, type, x, mask, image, v, radius, rmass, angmom, body) as per create_atom()
          // should thus ideally edit this to reflect that
...
void FixClientGCMC::attempt_atomic_insertion_full()
{
  double lamda[3];
  ninsertion_attempts += 1.0;
  ...
  //int random_type = rand() % atom->ntypes + 1; // randomly choosing atom type to insert from ntypes
  int random_type = ngcmc_type; // randomly choosing atom type to insert from ntypes
  ...
  for (int a = 0; a < ngas; a++){
  	int b = local_gas_list[a];
    if (atom->type[b] == random_type){
    	count++;
    }
  }

  if (proc_flag) {
    printf("Choose type %d \n", random_type);
    atom->avec->create_atom(random_type,coord); 
    int m = atom->nlocal - 1;
  ...
  comm->borders();
  if (triclinic) domain->lamda2x(atom->nlocal+atom->nghost);
  //if (force->kspace) force->kspace->qsum_qsq(); // edit
  //if (force->pair->tail_flag) force->pair->reinit(); // edit
  ...
  gas_mass = atom->mass[random_type];
  printf("mass is %f\n", gas_mass);
  double mu;
  mu = chemical_potential;
  printf("mu is %f\n", mu);
...
/*  beta = 1.0/(force->boltz*reservoir_temperature);
  ...
  double ran_num_ins = random_equal->uniform();

  if (energy_after < MAXENERGYTEST &&
      ran_num_ins <
      zz*volume*exp(beta*(energy_before - energy_after))/(count+1)) {

  	printf("Accepting move as random number %f is less than RHS %f\n",ran_num_ins,zz*volume*exp(beta*(energy_before - energy_after))/(count+1));
...
/* ----------------------------------------------------------------------
------------------------------------------------------------------------- */
// have not edited the functions below to enable molecule exchange

/*void FixClientGCMC::attempt_molecule_translation_full()
{
  ntranslation_attempts += 1.0;
...
/*void FixClientGCMC::attempt_molecule_rotation_full()
{
  nrotation_attempts += 1.0;
...
/*void FixClientGCMC::attempt_molecule_deletion_full()
{
  ndeletion_attempts += 1.0;
...
/*void FixClientGCMC::attempt_molecule_insertion_full()
{
  double lamda[3];
  ninsertion_attempts += 1.0;
...
/*double FixClientGCMC::energy(int i, int itype, tagint imolecule, double *coord)
{
...
/*double FixClientGCMC::molecule_energy(tagint gas_molecule_id)
{
...
double FixClientGCMC::energy_full() 
{
  // from fix_gcmc.cpp
  ...
  // commenting out below, as VASP computes forces and energy, not LAMMPS
  ...
  // largely from post_force(int vflag) in fix_client_md.cpp 
  ...
  double total_energy = receive_fev(vflag);
  printf("Energy from VASP is %f\n", total_energy);

  if (server_error) {
    char str[64];
    sprintf(str,"Fix client/gcmc received server error %d\n",server_error);
    error->all(FLERR,str);
  }
...
int FixClientGCMC::pick_random_gas_atom()
{
  // from fix_gcmc.cpp
  int i = -1;
...
/*tagint FixClientGCMC::pick_random_gas_molecule()
{
  int iwhichglobal = static_cast<int> (ngas*random_equal->uniform());
...
/*void FixClientGCMC::toggle_intramolecular(int i)
{
...
void FixClientGCMC::update_gas_atoms_list()
{
...
/* ----------------------------------------------------------------------
   update the positions of atoms to that of CONTCAR from VASP
------------------------------------------------------------------------- */

void FixClientGCMC::send_recv_update_request()
{
  CSlib *cs = (CSlib *) lmp->cslib;

  int nfields = 0;

  // UPDATE send when CONTCAR required

  cs->send(UPDATE,nfields);

  update_gas_atoms_list_accepted();
}

/* ----------------------------------------------------------------------
   update the list of gas atoms using positions from VASP CONTCAR 
------------------------------------------------------------------------- */

void FixClientGCMC::update_gas_atoms_list_accepted()
{

  // update the coordinates and types from VASP CONTCAR
  // largely from receive_fev(vflag) of fix_client_md.cpp and 
  // update_gas_atoms_list() of fix_gcmc.cpp
  ...
  double *relaxed_coords = (double *) cs->unpack(COORDS);
  int *relaxed_types = (int *) cs->unpack(TYPES);
  double **x = atom->x;
  int *type = atom->type; 
  int nlocal = atom->nlocal;
  ...
  for (tagint id = 1; id <= natoms; id++) {
    ...
    if (m < 0 || m >= nlocal) j += 3;
      ...
      x[m][0] = relaxed_coords[j++];
      x[m][1] = relaxed_coords[j++];
      x[m][2] = relaxed_coords[j++];
      printf("%f %f %f\n", x[m][0], x[m][1], x[m][2]);
      type[m] = relaxed_types[m];
    }
  }

  printf("Updated gas atoms from CONTCAR \n");
...
------------------------------------------------------------------------- */
// from fix_gcmc.cpp
double FixClientGCMC::compute_vector(int n)
{
...
------------------------------------------------------------------------- */
// from fix_gcmc.cpp
double FixClientGCMC::memory_usage()
{
...
------------------------------------------------------------------------- */
// from fix_gcmc.cpp
void FixClientGCMC::write_restart(FILE *fp)
{
  int n = 0;
  double list[12];
...
------------------------------------------------------------------------- */
// from fix_gcmc.cpp
void FixClientGCMC::restart(char *buf)
{
  int n = 0;
...
void FixClientGCMC::grow_molecule_arrays(int nmolatoms) {
...
void FixClientGCMC::setup(int vflag) 
{
  CSlib *cs = (CSlib *) lmp->cslib;
  ...
  cout << "Storing energy from setup"; cout << "\n";
  ofstream fout;
  fout.precision(10);
  fout.open ("energies_past.txt",ios::app);
  fout << total_energy;
  fout.close();
  ...
  // update atom positions to that of CONTCAR from VASP
...
------------------------------------------------------------------------- */
// from fix_client_md.cpp
double FixClientGCMC::compute_scalar()
{
  printf("eng in compute_scalar() is %f\n", eng);
  return eng;
}
...
------------------------------------------------------------------------- */
// from fix_client_md.cpp
void FixClientGCMC::pack_coords()
{
  double **x = atom->x;
  int nlocal = atom->nlocal;
...
------------------------------------------------------------------------- */
// largely from fix_client_md.cpp
double FixClientGCMC::receive_fev(int vflag) 
{
  CSlib *cs = (CSlib *) lmp->cslib;
  ...
  printf("unpacked energy from VASP = %f\n", cs->unpack_double(ENERGY));
  eng = econvert * cs->unpack_double(ENERGY);
  printf("energy in receive_fev is %f\n", eng);
  ...
  server_error = 0; 
  if (nfield == 2) server_error = cs->unpack_int(ERROR); 
}

double FixClientGCMC::get_prev_energy()
{
  ifstream fin;
  fin.open("energies_past.txt");
  if(fin.is_open()) {
  ...
  energy_stored = atof(lastLine.c_str());
  fin.close();
  return energy_stored;
  }
}

void FixClientGCMC::storing_energy(double energy_stored)
{
  ofstream fout;
  fout.precision(10);
  fout.open ("energies_past.txt",ios::app);
